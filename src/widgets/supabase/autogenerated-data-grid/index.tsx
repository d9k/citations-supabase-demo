...
type EditableById = { [id: number]: boolean };
...
export const SupabaseAutogeneratedDataGrid = (
  ...
  // TODO type
  const editableData: RlsEditForTable[] = (editableDataResult as any)?.data ||
    [];

  const editableById: EditableById = useMemo(() =>
    Object.fromEntries(
      editableData.map(({ id, editable }) => [id, editable]),
    ), [editableData]);

  const hasEditableInfo = editableData.length > 0;
  ...
  const {
    ...
    fieldsNames,
    fieldsNamesRequired,
  } = useFieldsNames({ hasEditableInfo, tableSchema });
  ...
  const rows = useMemo(() => {
    ...
    if (hasEditableInfo) {
      result = result.map((r) => ({
        ...r,
        [COLUMN_NAME_EDITABLE]: editableById[r.id] ? 'yes' : '',
      }));
    }
  ...
  }, [currentRow, editableById, hasEditableInfo, tableData]);
