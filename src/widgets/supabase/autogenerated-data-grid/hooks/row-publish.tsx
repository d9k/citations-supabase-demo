...
export type UseOnRowPublishArgs = {
...
export const HOOK_NAME = 'useOnRowPublish';

export const useOnRowPublish = ({
  onReloadRequired,
  rowKeyGetter,
  // setRowToDelete,
  tableName,
  ...
  return useCallback(async (rowToPublish: RecordAny) => {
    ...
    try {
      result = await supabase?.rpc('content_item_publish', {
        _table_name: tableName,
        _id: parseInt(`${id}`, 10),
      });

      console.log(
        `${HOOK_NAME}: publish result:`,
        result,
        'publishing row:',
        rowToPublish,
      );
    ...
    if (errorText) {
      ...
      notifications.show({
        message: `Successfullly published`,
      });
